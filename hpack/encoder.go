...
import (
	...
	"minimalist-http2/hpack/integer_representation"
)

func (frame *IndexedHeader) Encode() (buf *swrap.SWrap) {
	buf = swrap.Make(integer_representation.Encode(frame.Index, 7))
	(*buf)[0] += 0x80
	if frame.Index == 0 {
		// TODO: Encoding Error
	}
...
func (frame *IndexedLiteral) Encode() (buf *swrap.SWrap) {
	...
	case WITH:
		buf = swrap.Make(integer_representation.Encode(frame.Index, 6))
		(*buf)[0] += 0x20
	case WITHOUT:
		buf = swrap.Make(integer_representation.Encode(frame.Index, 4))
	case NEVER:
		buf = swrap.Make(integer_representation.Encode(frame.Index, 4))
		(*buf)[0] += 0x8
	}
	...
	// No Huffman
	buf.Merge(integer_representation.Encode(frame.ValueLength, 7))
...
func (frame *IndexedLiteral) EncodeHuffman() (buf *swrap.SWrap) {
	...
	length = integer_representation.Encode(uint32(len(encoded)), 7)
...
func (frame *StringLiteral) Encode() (buf *swrap.SWrap) {
	...
	// No Huffman
	buf.Merge(integer_representation.Encode(frame.NameLength, 7))
...
func (frame *StringLiteral) EncodeHuffman() (buf *swrap.SWrap) {
	...
	logger.Trace("huffman encoded %s %v", frame, *buf)
	return buf
}
...
func (frame *DynamicTableSizeUpdate) Encode() (buf *swrap.SWrap) {
	...
	buf.Merge(integer_representation.Encode(frame.MaxSize, 4))
	return buf
}
